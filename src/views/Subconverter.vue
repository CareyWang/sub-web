  ...
  methods: {
    ...
    loadPublicConfig() {
      ...
      try {
        ...
        axios.get('/config/backendOptions.json')
            .then(response => {
        ...
        axios.get('/config/remoteConfig.json')
            .then(response => {
              if (!response.data) return;
              const remoteConfig = JSON.parse(JSON.stringify(response.data));
              if (Array.isArray(remoteConfig)) {
                this.options.remoteConfig.unshift(...remoteConfig);
              }
            })
            .catch(err => {
              throw new Error(err)
